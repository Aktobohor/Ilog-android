...
/**
 * Activity that extends {@link ContributionActivity} and is intended to display the content of a {@link Question}.
 */
public class QuestionActivity extends ContributionActivity {
     ...
     *     {@link Question} in the database, start the {@link HomeActivity} and close this one using {@link #finish()}.</li>
     * </ul>
        ...
        switch (item.getItemId()) {
            case R.id.next:
                if(nextItem.getTitle().equals(getString(R.string.finish))) {
                    ...
                    Answer answer = new Answer(contribution.getInstanceTime(), System.currentTimeMillis(), contribution.getNotifiedTime(), System.currentTimeMillis()-startingTime, answersToJSONArray(), payloadToJSONArray(), contribution.getInstanceid(), Answer.TYPE_TIMEDIARY, Answer.SYNCHRONIZATION_FALSE, Answer.SYNCHRONIZATION_FALSE);
                    iLogApplication.db.addAnswer(answer);
                    iLogApplication.db.updateQuestion((Question) contribution, Question.STATUS_ANSWERED);
