...
import it.unitn.disi.witmee.sensorlog.broadcastreceivers.BatteryPowerBroadcastReceiver;
import it.unitn.disi.witmee.sensorlog.model.metalog.SM;
import it.unitn.disi.witmee.sensorlog.model.system.ST;

/**
 * Class that implements a {@link Runnable} that manages the data collection of the {@link it.unitn.disi.witmee.sensorlog.model.system.BC} event. The data
 * collection occurs through a {@link BatteryPowerBroadcastReceiver}. It is registered/unregistered in this class and the data of type
 * {@link it.unitn.disi.witmee.sensorlog.model.system.BC} event is generated in it. In this case we leverage on system level {@link android.content.BroadcastReceiver},
 * specifically {@link Intent#ACTION_POWER_CONNECTED} and {@link Intent#ACTION_POWER_DISCONNECTED} that respectively are triggered when device is attached and detatched
 * to/from a power source.
 * @author Mattia Zeni
 */
public class BatteryChargeRunnable implements Runnable {

    private volatile boolean isStopped = false;
    private static BatteryPowerBroadcastReceiver batteryPowerBroadcastReceiver = null;
    private static int SENSOR_ID = iLogApplication.BATTERY_CHARGE_ID;
     ...
     *     <li>Initializes the {@link #batteryPowerBroadcastReceiver} variable where the detection of the mode change will occur</li>
     *     <li>Registers the receiver {@link #batteryPowerBroadcastReceiver} using the {@link android.content.Context#registerReceiver(BroadcastReceiver, IntentFilter)}
     *         method for actions of type {@link Intent#ACTION_POWER_CONNECTED} and {@link Intent#ACTION_POWER_DISCONNECTED}</li>
     * </ul>
     */
    public void run() {
        if(iLogApplication.sensorLoggingState.get(SENSOR_ID) != null) {
            ...
            if(!iLogApplication.sensorLoggingState.get(SENSOR_ID)) {
                ...
                batteryPowerBroadcastReceiver = new BatteryPowerBroadcastReceiver();
                iLogApplication.getAppContext().registerReceiver(batteryPowerBroadcastReceiver, new IntentFilter(Intent.ACTION_POWER_CONNECTED));
                iLogApplication.getAppContext().registerReceiver(batteryPowerBroadcastReceiver, new IntentFilter(Intent.ACTION_POWER_DISCONNECTED));
            }
        }
    }
     ...
     * <ul>
     *     <li>Unregisters the {@link #batteryPowerBroadcastReceiver} using the {@link android.content.Context#unregisterReceiver(BroadcastReceiver)} method to stop receiving updates</li>
    ...
    public void stop() {
        if(iLogApplication.sensorLoggingState.get(SENSOR_ID) != null) {
            if(!isStopped() && batteryPowerBroadcastReceiver !=null) {
                try {
                    iLogApplication.getAppContext().unregisterReceiver(batteryPowerBroadcastReceiver);
                } catch (Exception e) {

                }
            }
            batteryPowerBroadcastReceiver =null;
     ...
     * <ul>
     *     <li>Unregister the {@link #batteryPowerBroadcastReceiver} using the {@link android.content.Context#unregisterReceiver(BroadcastReceiver)} method to stop receiving updates</li>
     *     <li>Registers the receiver {@link #batteryPowerBroadcastReceiver} using the {@link android.content.Context#registerReceiver(BroadcastReceiver, IntentFilter)}
    ...
    public void restart() {
        if(iLogApplication.sensorLoggingState.get(SENSOR_ID) != null) {
            if(!isStopped()) {
                if (batteryPowerBroadcastReceiver != null) {
                    try {
                        iLogApplication.getAppContext().unregisterReceiver(batteryPowerBroadcastReceiver);
                    } catch (Exception e) {
